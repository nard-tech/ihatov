...
RSpec.describe '辞書の検証 Dictionary validation' do
  ...
  context '任意の属性が省略されているとき when optional fields are absent' do
    ...
    it '座標と年代を nil として読み込むこと loads absent coordinates and years as nil' do
  ...
  context '種別が異なる項目で ID が一致するとき when IDs match across categories' do
    before { kenji_works[1]['quotes'][0]['id'] = 'shared' }

    it '種別間の同一 ID を許可すること permits matching IDs across categories' do
  ...
  context '青空文庫以外の出典のとき when the source is outside Aozora' do
    ...
    it '青空文庫 ID を nil として読み込むこと loads the Aozora ID as nil' do
  ...
  describe '辞書フィールドの検証 dictionary field validation' do
    context '青空文庫 ID が不正のとき when the Aozora ID is invalid' do
      before { kenji_works[0]['edition']['aozora_id'] = 0 }

      it 'DataError を返すこと raises DataError' do
        expect { repository }.to raise_error(Ihatov::DataError, /aozora_id/)
      end
    end
    context '文章形式が異なっても ID が重複するとき when quote IDs repeat across forms' do
    ...
    context '座標があっても real がないとき when real is absent despite coordinates' do
    ...
    context '採用版の URL がないとき when an edition URL is absent' do
    ...
    context 'ID の形式が不正なとき when an ID is malformed' do
    ...
    context 'URL のスキームが不正なとき when a URL scheme is invalid' do
    ...
    context '収録配列に nil を指定するとき when a collection is nil' do
    ...
    context '作者をまたいで作品 ID が重複するとき when work IDs repeat across authors' do
    ...
    context '分類をまたいで登場者 ID が重複するとき when being IDs repeat across kinds' do
  ...
  describe 'YAML の安全な読み込み safe YAML loading' do
