...
module Ihatov
  ...
  # @note 抽選候補がなければ NotFoundError、不正な引数なら ArgumentError。
  #   Empty samples raise NotFoundError; invalid arguments raise ArgumentError.
  module API
    # @param options [Hash] 絞り込み条件と任意の乱数生成器（Collection#sample を参照）
    ...
    # @param options [Hash] 絞り込み条件。indent: true は字下げ詩を半角2スペースで返す。
    #   false は字下げのない詩、文字列は字下げ詩とその字下げ単位を指定する。
    #   省略時はすべての詩が対象。random: には独立した Random を指定できる。
